...
import medicalBolletImg from "@/assets/medical-bollet.png";
...
const BestSellers = () => {
  ...
  const sliderData = Array.from({ length: 10 }, (_, index) => ({
    id: index + 1,
    price: 5 + index,
    onSale: index === 1 ? true : false,
    hasDiscount: index === 0 ? true : false,
    inCart: index === 0 ? true : false,
  ...
  return (
    <div className="flex flex-col gap-11 max-md:gap-8 max-md:py-8 max-md:ps-4 md:py-16 md:ps-6 lg:p-[64px_32px]">
      <div className="flex items-center justify-between max-md:pe-4 md:pe-6">
        <h5 className="heading5 max-md:heading6">{t("home.bestSellers")}</h5>
        ...
        >
          {sliderData.slice(0, 10).map((item) => (
            <SwiperSlide key={item.id}>
              <ProductCard item={item} productImg={medicalBolletImg} />
            </SwiperSlide>
          ))}
        </Swiper>
      </div>
