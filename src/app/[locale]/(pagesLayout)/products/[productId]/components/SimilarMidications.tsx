...
const SimilarMidications = () => {
  ...
  return (
    <div className="flex flex-col gap-11 max-md:gap-8 max-md:py-8 max-md:ps-4 md:py-16 md:ps-6 lg:p-[64px_32px]">
      <div className="flex items-center justify-between max-md:pe-4 md:pe-6 lg:pe-0">
        <h5 className="heading5 max-md:heading6">
          {t("singleProduct.similarMedications")}
        </h5>
        ...
        >
          {sliderData.slice(0, 10).map((item) => (
            <SwiperSlide key={item.id}>
              <ProductCard item={item} productImg={PanadolImg} />
            </SwiperSlide>
          ))}
        </Swiper>
      </div>
...
export default SimilarMidications;
